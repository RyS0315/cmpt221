...
<h1>Update a Found Item!</h1>

<!-- Get inputs from the user. -->
<form action="editFound.php" method="POST">
<table border=1>
...
if ($_SERVER[ 'REQUEST_METHOD' ] == 'POST') {
	...
	*/
    ...
    if( !empty($status) && !empty($description) && !empty($finder)&& !empty($id)){
	   $status = 'found';
        $created_date='now()';
        update_record_found($dbc,$description,$finder,$status,$room, $id);
    }
}
